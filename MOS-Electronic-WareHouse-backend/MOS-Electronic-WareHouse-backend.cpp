...
static void* doit(void* a)
{
    ...
    for (;;)
    {
        ...
        char* InURL = FCGX_GetParam("REQUEST_URI", request.envp);
        std::cout << "REQUEST_URI name: " << InURL << std::endl;
        if (method == "GET") {
            if (InURL == "/data") {
                DataBase db("test.db");
                FCGX_PutS(db.SelectData("SELECT * FROM WareHouse Where TypeCell = 'Midlle' AND Empty = 0 ORDER BY HeightCell").c_str(), request.out);
            }
            else {
                ...
                FCGX_PutS(InURL ? InURL : "?", request.out);
                FCGX_PutS("</i></p>\r\n", request.out);
                FCGX_PutS("</body>\r\n", request.out);
                FCGX_PutS("</html>\r\n", request.out);
            }
